...
type GetPostListForUserParams struct {
	UserID uuid.UUID
	Limit  int32
}

func (q *Queries) GetPostListForUser(ctx context.Context, arg GetPostListForUserParams) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, getPostListForUser, arg.UserID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
